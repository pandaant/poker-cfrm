...
public:
  ...
  EHSAbstractionGenerator(std::ofstream &dump_to, int_c buckets_per_round, int_c samples_per_round,
                          ecalc::Handranks *hr, int nb_threads = 1)
      : AbstractionGenerator(dump_to), nb_buckets(buckets_per_round), nb_samples(samples_per_round),
...
public:
  ...
  EMDAbstractionGenerator(std::ofstream &dump_to,int_c buckets_per_round, int_c samples_per_round,
                          int_c num_history_points, ecalc::Handranks *hr,
                          int nb_threads = 1)
      : AbstractionGenerator(dump_to), nb_buckets(buckets_per_round), nb_samples(samples_per_round),
        calc(nb_threads), num_history_points(num_history_points),
...
public:
  ...
  OCHSAbstractionGenerator(std::ofstream &dump_to, int_c buckets_per_round, int_c samples_per_round,
                          int_c num_opponent_clusters, ecalc::Handranks *hr,
                          int nb_threads = 1)
      : AbstractionGenerator(dump_to),  nb_buckets(buckets_per_round), nb_samples(samples_per_round),
        calc(nb_threads), num_opponent_clusters(num_opponent_clusters),
  ...
  virtual void generate_round(int round, nbgen &rng) {
    ...
    std::cout << "generating " << num_opponent_clusters[round] << " opponent clusters\n";
    uint8_t cards[7];
    ecalc::SingleHandlist handlist(poker::Hand(1, 2));
    ...
    std::vector<std::vector<poker::Hand>> opp_range(num_opponent_clusters[round]);
    std::vector<ecalc::ArrayHandlist*> opp_clusters(num_opponent_clusters[round]);
    ...
    for (int t = 0; t < nb_threads; ++t) {
      ...
      eval_threads[t] =
          std::thread([t, round, accumulator, &features, &board_card_sum,
                       &thread_block_size, &rng, &round_size, &opp_clusters,
                       this] {
            uint8_t cards[7];
            std::vector<ecalc::card> board(board_card_sum[round]);
            ecalc::SingleHandlist handlist(poker::Hand(1, 2));
              ...
              if (t == 0 && i % 10000 == 0)
                ...
                for (unsigned ochs = 0; ochs < opp_clusters.size(); ++ochs) {
                  try {
                    ...
                    std::cout << poker::Hand(cards[0]+1, cards[1]+1).str() << ": lol i got it. setting shit cluster " << ochs
                              << " to zero\n";

                    features[i].histogram[ochs] = 0;
                  }
                  mass_sum += features[i].histogram[ochs];
                  //std::cout << "hist for " << poker::Hand(cards[0]+1, cards[1]+1).str() << " idx " << ochs << ": "<< features[i].histogram[ochs] << "\n";
                }
                // normalize
                for (unsigned ochs = 0; ochs < opp_clusters.size(); ++ochs) {
...
public:
  ...
  virtual void generate_round(int round, nbgen &rng) {
    ...
    for (unsigned i = 0; i < nb_entries; ++i) {
      dump_to->write(reinterpret_cast<const char *>(&i),
                     sizeof(nb_entries));
    }
  }
};
...
public:
  MixedAbstractionGenerator(std::ofstream &dump_to, std::vector<AbstractionGenerator *> generators)
      :  AbstractionGenerator(dump_to), generators(generators){
    //for (size_t i = 0; i < buckets.size(); ++i) {
      //dump_f.write(reinterpret_cast<const char *>(&nb_buckets[i]),
                   //sizeof(nb_buckets[i]));
    //}
  ...
  virtual void generate_round(int round, nbgen &rng) {}
  //void dump(int round, std::vector<unsigned> buckets) {
