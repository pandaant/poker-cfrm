...
void kmeans_emd(unsigned nb_clusters, std::vector<DATAPOINT> &dataset,
  ...
  do {
    ...
    // double distance =
    // emd_distance(dataset[i].histogram, dataset[j].histogram, cost_mat);
    // cluster_element_distance_sums[dataset[i].cluster][ +=
    //}
    //}

    std::cout << "changed: " << changed << "\n";

    // std::cout << "calculating distance between buckets:\n";
    // for (unsigned o = 0; o < nb_clusters; ++o) {
    // for (unsigned u = 0; u < nb_clusters; ++u) {
    // std::cout << "d(" << o << ","  << u << ") = " <<
    // emd_distance(centers[o].histogram, centers[u].histogram, cost_mat) <<
    // "\n";
    //}
    //}

    // for (unsigned b = 0; b < nb_clusters; ++b) {
    // centers[b] = (1.0 / cluster_element_counter[b]) *
    // cluster_element_sums[b];
    //}
  } while ((1.0 * changed) / dataset.size() > epsilon);
}
